...
def exercise01():
    ...
    import random
    df = pd.DataFrame({'Column 1': [random.randint(1, 100) for i in range(3)],
                   'Column 2': [random.randint(1, 100) for i in range(3)],
                   'Column 3': [random.randint(1, 100) for i in range(3)],
                   'Column 4': [random.randint(1, 100) for i in range(3)]})
...
def exercise07(sd,m,s):
    ...
    # ------ Place code below here \/ \/ \/ ------
    random_numbers = np.random.normal(loc=m, scale=sd, size=(s, s))
...
def exercise13(n):
    ...
    s = pd.Series(np.random.randint(0, n, size=n), index=pd.date_range(start='1/1/2010', periods=n, freq='D'))
...
# test size 100000 is taking too long to run, reduced to size 100 for testing purpose.    
    def test_exercise07(self):
        print('Testing exercise 7')
        z = exercise07(10,5,100)
        self.assertEqual(z.shape[0], 100)
        self.assertLessEqual(np.average(z), 5.2)
        self.assertGreaterEqual(np.average(z), 4.7)
        z = exercise07(5,10,100)
        self.assertEqual(z.shape[0], 100)
